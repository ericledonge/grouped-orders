...
export default async function PaymentPage({ params }: PaymentPageProps) {
  ...
  const validatedWishes =
    basket.wishes?.filter((w) => w.status === "validated") || [];
  const paidWishes =
    basket.wishes?.filter(
      (w) => w.paymentStatus === "sent" || w.paymentStatus === "received",
    ) || [];
  ...
  const wishesToPay = validatedWishes.filter(
    (w) => w.paymentStatus === "pending" || !w.paymentStatus,
  );
        ...
        <Card className="border-green-500 bg-green-50/50 dark:bg-green-950/20">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center max-w-md">
              Votre paiement a été marqué comme envoyé. L'administrateur le
              confirmera dès réception.
            </p>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {unitPrice.toFixed(2)} € + {shippingShare.toFixed(2)}{" "}
                          € port
                          {customsShare > 0 &&
